...
void Emit(int i) {
    vec3 pos = vec3(gs_in[i].pos);
    float turbulence_value = turbulence(pos * 6.0);
    turbulence_value = (turbulence_value * 1.0);

    vec3 normal = normalize(vec3(gl_in[i].gl_Position));
    float distance = random(vec2(gl_in[i].gl_Position)); //normal * (gl_in[i].gl_Position.y > 0.0 ? 1.0 : 10.0);

    gl_Position = gl_in[i].gl_Position;//; + vec4(distance, distance, distance, 0.0);
    gs_out.pos = vec4(pos, 1.0);
